...
export default function ReemplazoEtiquetas({ generacionId, piezas }: { generacionId: string; piezas: Pieza[] }) {
  ...
  const [registrados, setRegistrados] = useState<string[] | null>(null);
  ...
  async function registrarYGenerar() {
    ...
    try {
      ...
      setRegistrados(ids);
      window.open(`/api/codigos-barra/${generacionId}/reemplazo/pdf?piezas=${ids.join(",")}&formato=hoja`, "_blank");
      setListo(`${ids.length} etiqueta(s) marcadas como reimpresas. Descarga abajo en el formato que necesites.`);
    ...
    <div className="space-y-4">
      ...
      {seleccion.size > 0 && (
        <div className="flex flex-wrap items-end gap-3 border-t border-brand-150 pt-4">
          ...
          <button type="button" onClick={registrarYGenerar} disabled={enviando} className="btn-primary text-sm disabled:opacity-50">
      ...
      {listo && (
        <div className="space-y-2">
          <p className="text-sm text-green-700">{listo}</p>
          {registrados && (
            <div className="flex flex-wrap gap-2">
              <a
                href={`/api/codigos-barra/${generacionId}/reemplazo/pdf?piezas=${registrados.join(",")}&formato=hoja`}
                target="_blank"
              ...
              <a
                href={`/api/codigos-barra/${generacionId}/reemplazo/pdf?piezas=${registrados.join(",")}&formato=termica`}
                target="_blank"
                className="btn-secondary text-xs"
              >
