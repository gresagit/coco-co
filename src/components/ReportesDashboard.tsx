...
type Props = {
  kpis: { productos: number; insumos: number; proveedores: number; sucursales: number; ordenesCompra: number; ordenesProduccion: number; ventas30: number; movimientos30: number; gastos30: number };
  ventasPorDia: { label: string; value: number }[];
  movimientosPorTipo: { label: string; value: number }[];
  gastosPorMes: { label: string; value: number }[];
};
const reportes = [
...
export default function ReportesDashboard({ kpis, ventasPorDia, movimientosPorTipo, gastosPorMes }: Props) {
  ...
  const maxGasto = Math.max(...gastosPorMes.map((item) => item.value), 1);
  ...
  return <div className="space-y-8 animate-fade-in-up">
    ...
    <div className="grid grid-cols-2 lg:grid-cols-5 gap-3 stagger-fade"><Kpi label="Ventas 30 días" value={`$${kpis.ventas30.toLocaleString("es-MX", { maximumFractionDigits: 0 })}`} /><Kpi label="Gastos 30 días" value={`$${kpis.gastos30.toLocaleString("es-MX", { maximumFractionDigits: 0 })}`} /><Kpi label="Movimientos" value={kpis.movimientos30} /><Kpi label="Órdenes de compra" value={kpis.ordenesCompra} /><Kpi label="Órdenes de producción" value={kpis.ordenesProduccion} /></div>
    ...
    <section className="card"><div className="mb-5"><h2 className="font-semibold">Gastos por mes</h2><p className="text-xs text-brand-400 mt-1">Últimos 6 meses</p></div><div className="h-40 flex items-end gap-2 sm:gap-3 border-b border-brand-150 pt-4">{gastosPorMes.map((item) => <div key={item.label} className="flex-1 h-full flex flex-col justify-end items-center gap-2"><div className="w-full max-w-10 bg-brand-500 rounded-t-md transition-all" style={{ height: `${Math.max((item.value / maxGasto) * 88, item.value ? 12 : 2)}%` }} title={`${item.label}: $${item.value.toFixed(2)}`} /><span className="text-[11px] text-brand-400">{item.label}</span></div>)}</div></section>
