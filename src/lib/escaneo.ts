...
export async function confirmarPiezaPorFolio(
  ...
  const { data: bomItems } = await db.from("bom").select("*").eq("producto_id", pieza.producto_id);
  for (const item of bomItems || []) {
    const cantidadNecesaria = Number(item.cantidad_por_unidad);
    if (cantidadNecesaria <= 0) continue;

    if (item.insumo_id) {
      await descontarInsumo(db, item.insumo_id, sucursalId, cantidadNecesaria, { piezaId: pieza.id });
